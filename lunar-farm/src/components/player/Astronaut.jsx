...
export default function Astronaut({ position = [0, 0, 0], ...props }) {
  ...
  useFrame(() => {
    if (groupRef.current && bodyRef.current) {
      // Sincronizar la posición visual con la física
      groupRef.current.position.copy(bodyRef.current.position)
      groupRef.current.quaternion.copy(bodyRef.current.quaternion)
    }
  })
